...
def checkChirality(rots,trans,oldpoints, newpoints,alpha):
	...
	for i in range(oldpoints.shape[0]):
		...
		for j in range(2):
			...
			Anew = np.squeeze(np.array([[oldpoint[0] * Horigin[2, :] - Horigin[0, :]], [oldpoint[1] * Horigin[2, :] - Horigin[1, :]],
			...
			# Xdash= H\X;  % linalg.inv(H) is taken to calculate as again Hdash is wrt to the other frame.
